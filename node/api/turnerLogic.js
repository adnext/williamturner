...
exports.turnerLogic = function(originalText) {
    ...
    //var separatorWordVowels = /([aeoui]*[a-z&&[^aeiou]]+)/i; //vowels followed by consonants

    var turnedText = "";
    var resultSentence= "";
    var consonants = [];

    var sentences = originalText.split(separtorSentences); // split into sentences
    ...
    for (var i = 0; i < sentences.length; i++) {
        ...
        var words = sentences[i].split(separtorWords); // split into words

        //for every word
        for (var n = 0; n < words.length; n++) {
            var letters = words[n].split(separatorWordVowels); // split into letter groups

            //for every letter
            for (var k = 0; k < letters.length; k++) {
                // if current letters contain consonants, replace them with a placeholder and store them in a separate array (consonants)
        ...
        // @TODO randomize? / pop gives pretty results
